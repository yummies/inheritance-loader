...
export default class {
    constructor(sourcePath) {
        ...
        this.remainingLayers = config.slice(0, this.sourceLayerIndex + 1);
    }

    getSourceLayerIndex() {
        for (let i = 0; i < config.length; i++) {
            if (this.sourcePath.indexOf(config[i].path) === 0) {
                return i;
            }
        }
    ...
    collectPossiblePaths(required) {
        const out = [];
        const layers = required.opts.raw ? this.remainingLayers : config;
    ...
    get(requiredString) {
        ...
        if (parsedRequire.opts.raw) {
            yummifyMethod = 'yummifyChainRaw';
        } else if (parsedRequire.opts.class) {
            yummifyMethod = 'yummifyChainClass';
        } else {
            yummifyMethod = 'yummifyChain';
        }
