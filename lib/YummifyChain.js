...
export default class {
    constructor(sourcePath, layers) {
        ...
        this.remainingLayers = this.layers.slice(0, this.sourceLayerIndex + 1);
    }

    getSourceLayerIndex() {
        for (let i = 0; i < this.layers.length; i++) {
            if (this.sourcePath.indexOf(this.layers[i].components) === 0) {
                return i;
            }
        }
    ...
    collectPossiblePaths(required) {
        const out = [];
        const layers = required.opts.raw ? this.remainingLayers : this.layers;
