...
export default class {
    constructor(sourcePath, config) {
        ...
        this.remainingLayers = this.config.slice(0, this.sourceLayerIndex + 1);
    }

    getSourceLayerIndex() {
        for (let i = 0; i < this.config.length; i++) {
            if (this.sourcePath.indexOf(this.config[i].components) === 0) {
                return i;
            }
        }
    ...
    collectPossiblePaths(required) {
        const out = [];
        const layers = required.opts.raw ? this.remainingLayers : this.config;
